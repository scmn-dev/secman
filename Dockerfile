...
LABEL abdcodex/test=../../tools/tests/tool-docker.yml
USER root
...
LABEL abdcodex/test=../../tools/tests/tool-brew.yml

RUN $UPD_s
ENV TRIGGER_BREW_REBUILD=1
RUN mkdir ~/.cache && /bin/bash -c "$(curl -fsSL $GITHUB_URL/Homebrew/install/HEAD/install.sh)"
