...
// PassDirExists is used to determine if the secman
...
// PassConfigExists is used to determine if the secman config
// file exists in the user's secman directory.
...
// exists in the user's secman directory.
...
func GetVault() (s SiteFile) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Fatalf("Could not open site file. Run secman init.: %s", err.Error())
		}
