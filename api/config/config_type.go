		...
		Content: []*yaml.Node{
			{
				...
				Content: []*yaml.Node{
					...
					{
						HeadComment: "What editor sm should run when creating issues, pull requests, etc. If blank, will refer to environment.",
						Kind:        yaml.ScalarNode,
						Value:       "editor",
					},
					{
					...
					{
						HeadComment: "Aliases allow you to create nicknames for sm commands",
						Kind:        yaml.ScalarNode,
						Value:       "aliases",
					},
					{
...
func (c *fileConfig) GetWithSource(hostname, key string) (string, string, error) {
	if hostname != "" {
		...
		if hostValue != "" {
			...
			return hostValue, "~/.config/sm/hosts.yml", nil
		}
	}

	// TODO: avoid hardcoding this
	defaultSource := "~/.config/sm/config.yml"
...
func (c *fileConfig) Hosts() ([]string, error) {
	...
	sort.SliceStable(hostnames, func(i, j int) bool { return hostnames[i] == common.Default() })
