...
const wcwidth = require("./lib/wc");
const { logSymbols } = require("./lib/log");
const { isInteractive } = require("./lib/is-interactive");
const { isUnicodeSupported } = require("./lib/unicode");
...
class Spinner {
  ...
  set spinner(spinner) {
    ...
    if (typeof spinner === "object") {
      ...
      throw new Error(
        `There is no built-in spinner named '${spinner}'. See https://github.com/scmn-dev/secman/blob/main/tools/spinner/spinners.json for a full list.`
      );
    }
  ...
  stopAndPersist(options = {}) {
    ...
    this.stop();
    this.stream.write(
      `${this.getFullPrefixText(prefixText, " ")}${
        options.symbol || " "
      }${fullText}\n`
    );
